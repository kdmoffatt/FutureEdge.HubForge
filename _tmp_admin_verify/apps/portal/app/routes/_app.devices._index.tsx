...
type DevicesItem = { id: string; name: string; status: string; createdAt: string };

export default function DevicesListPage() {
  const [items, setItems] = useState<DevicesItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const tenantId = localStorage.getItem('tenantId') ?? '';
    const token = localStorage.getItem('token') ?? '';
    fetch(API + '/v1/devices', { headers: { 'x-tenant-id': tenantId, authorization: 'Bearer ' + token } })
      .then((res) => (res.ok ? res.json() : { items: [] }))
      .then((data: { items?: DevicesItem[] }) => setItems(data.items ?? []))
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>Devices</h2>
        <Link to="/devices/new" style={{ background: '#2563eb', color: '#fff', borderRadius: 8, textDecoration: 'none', padding: '8px 12px' }}>+ New</Link>
      </div>
          ...
          <tbody>
            ...
            {items.map((item) => (
              <tr key={item.id} style={{ borderTop: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: 12 }}><Link to="/devices/${item.id}">View</Link></td>
              </tr>
            ))}
          </tbody>
        </table>
