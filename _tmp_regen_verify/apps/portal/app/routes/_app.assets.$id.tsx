...
type AssetsItem = { id: string; name: string; status: string; createdAt: string; updatedAt: string };

export default function AssetsDetailPage() {
  const params = useParams();
  const [item, setItem] = useState<AssetsItem | null>(null);

  useEffect(() => {
    if (!params.id) return;
    const tenantId = localStorage.getItem('tenantId') ?? '';
    const token = localStorage.getItem('token') ?? '';
    fetch(API + '/v1/assets/' + params.id, { headers: { 'x-tenant-id': tenantId, authorization: 'Bearer ' + token } })
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => setItem(data));
  }, [params.id]);

  if (!item) {
    return <p style={{ color: '#94a3b8' }}>Loading Assets...</p>;
  }
