...
export async function loader({ request }: LoaderFunctionArgs) {
  // TODO: validate session cookie / access token.
  // Replace this stub with requireAuth(request) once OIDC is configured.
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#f9fafb' }}>
      <aside style={{ width: 240, background: '#fff', borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column' }}>
        ...
        <nav style={{ padding: '1rem', flex: 1 }}>
          {[{ href: '/dashboard', label: 'Dashboard' }, { href: '/settings', label: 'Settings' }].map(
            ({ href, label }) => (
